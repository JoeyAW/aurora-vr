...
struct FrameRecorder {
  ...
  // See set_protected_offscreen_pass()'s doc comment in gfx.hpp. 0 == none
  // (RenderPass::id starts at 1).
  uint64_t protectedOffscreenPassId = 0;
  // Nesting inside a protected offscreen pass (e.g. VR's eye pass): lets a
  // caller open its own offscreen pass (e.g. water's background-capture
  // GXCreateFrameBuffer) from within an already-open protected pass without
  // losing track of it. Only one level of this nesting is tracked, matching
  // this file's existing single-slot suspendedEfbPass pattern.
  bool suspendedProtectedOffscreen = false;
  uint32_t suspendedProtectedPassIndex = UINT32_MAX;
  webgpu::TextureWithSampler suspendedProtectedOffscreenColor;
  webgpu::TextureWithSampler suspendedProtectedOffscreenDepth;
  Viewport suspendedProtectedViewport;
  ClipRect suspendedProtectedScissor;
...
void resolve_pass_into(TextureHandle texture, ClipRect rect, bool clearColor, bool clearAlpha, bool clearDepth,
  ...
  // REVISED (VR water-black investigation): this function's substitution
  // below always carries the SAME colorAttachments/depthStencilView forward
  // onto the new pass object it creates -- it only reseals the pass
  // WRAPPER (a new id), never the underlying render target. resolve_pass_
  // checked() (used by endEye() to close out the eye pass) already has a
  // fallback for exactly this: an id mismatch alone isn't treated as
  // failure if current_pass_color_view() still matches what beginEye()
  // opened. So letting protected passes go through the same split as any
  // other in-game GXCopyTex drain is safe: the eye's own target survives,
  // and this call's actual capture destination finally gets real data
  // during VR instead of being permanently starved.
  const bool wasProtectedPass = g_recorder.inOffscreen && g_recorder.protectedOffscreenPassId != 0 &&
                                current_render_passes()[g_recorder.currentRenderPass].id ==
                                    g_recorder.protectedOffscreenPassId;
  if (wasProtectedPass) {
    Log.debug("resolve_pass_into: substituting a protected pass (id={}) -- letting it through "
              "(colorView carries forward; resolve_pass_checked() at endEye() tolerates the id change).",
              g_recorder.protectedOffscreenPassId);
  }
  if (g_recorder.inOffscreen) {
    Log.warn("resolve_pass_into: substituting the current pass while an offscreen pass is open "
             "(pass id={}) -- this seals/replaces whatever pass is current without checking "
             "inOffscreen. If something else opened that offscreen pass and expects it to still be "
             "current later, it will be silently orphaned unless it verifies via "
             "current_pass_id()/resolve_pass_checked().",
             current_render_passes()[g_recorder.currentRenderPass].id);
  }
  ...
  if (wasProtectedPass) {
    ...
    // internal checks (e.g. begin_offscreen()'s own nesting detection,
    // which compares current_render_passes()[currentRenderPass].id against
    // this same variable) still recognize the eye pass as protected after
    // this split, instead of silently losing track of it.
    g_recorder.protectedOffscreenPassId = current_render_passes()[g_recorder.currentRenderPass].id;
  }
...
void begin_offscreen(uint32_t width, uint32_t height) {
  ...
  // enough state to resume it in end_offscreen() below, instead of it being
  // silently lost (the new pass pushed on top of it, offscreenColor/Depth
  // overwritten, and end_offscreen() falling back to the EFB-restore path
  // with no idea a protected pass was ever there).
  if (g_recorder.inOffscreen && g_recorder.protectedOffscreenPassId != 0 &&
      current_render_passes()[g_recorder.currentRenderPass].id == g_recorder.protectedOffscreenPassId) {
    g_recorder.suspendedProtectedOffscreen = true;
    g_recorder.suspendedProtectedPassIndex = g_recorder.currentRenderPass;
    g_recorder.suspendedProtectedOffscreenColor = g_recorder.offscreenColor;
    g_recorder.suspendedProtectedOffscreenDepth = g_recorder.offscreenDepth;
    g_recorder.suspendedProtectedViewport = g_recorder.cachedViewport;
    g_recorder.suspendedProtectedScissor = g_recorder.cachedScissor;
  }
...
void end_offscreen() {
  ...
  if (g_recorder.suspendedProtectedOffscreen) {
    ...
    // pass, not the EFB.
    g_recorder.suspendedProtectedOffscreen = false;
    g_recorder.currentRenderPass = g_recorder.suspendedProtectedPassIndex;
    g_recorder.offscreenColor = g_recorder.suspendedProtectedOffscreenColor;
    g_recorder.offscreenDepth = g_recorder.suspendedProtectedOffscreenDepth;
    g_recorder.suspendedProtectedOffscreenColor = {};
    g_recorder.suspendedProtectedOffscreenDepth = {};
    g_recorder.inOffscreen = true;
    g_recorder.cachedViewport = g_recorder.suspendedProtectedViewport;
    g_recorder.cachedScissor = g_recorder.suspendedProtectedScissor;
    push_command(CommandType::SetViewport, Command::Data{.setViewport = g_recorder.cachedViewport});
    push_command(CommandType::SetScissor, Command::Data{.setScissor = g_recorder.cachedScissor});
    return;
  }
...
namespace {
bool s_offscreenUsesNativeLogicalSize = false;
} // namespace

void set_offscreen_uses_native_logical_size(bool enabled) noexcept { s_offscreenUsesNativeLogicalSize = enabled; }

bool offscreen_uses_native_logical_size() noexcept { return s_offscreenUsesNativeLogicalSize; }

void set_protected_offscreen_pass(uint64_t passId) noexcept { g_recorder.protectedOffscreenPassId = passId; }

void clear_protected_offscreen_pass() noexcept { g_recorder.protectedOffscreenPassId = 0; }
...
// See gfx.hpp's doc comment for the full root cause: resolve_pass_into()
// seals whatever pass is current and substitutes a new one in its place,
// entirely independent of inOffscreen's state. A caller that opens an
// offscreen pass via create_pass() and needs to be sure the SAME pass is
// still current later should capture this right after create_pass()
// succeeds.
uint64_t current_pass_id() noexcept {
  if (!g_recorder.active() || g_recorder.currentRenderPass == UINT32_MAX) {
    return 0;
  }
  return current_render_passes()[g_recorder.currentRenderPass].id;
}

// Companion to current_pass_id(): the current pass's scene colorView, or
// null outside a pass. resolve_pass_into() always carries the same
// colorAttachments forward onto the pass object it substitutes in (new id,
// same underlying render target), so a caller whose expected pass id no
// longer matches can still check whether its real render target is the
// current one via this, rather than assuming an id mismatch always means
// the target is gone. See resolve_pass_checked() below.
wgpu::TextureView current_pass_color_view() noexcept {
  if (!g_recorder.active() || g_recorder.currentRenderPass == UINT32_MAX) {
    return nullptr;
  }
  return current_render_passes()[g_recorder.currentRenderPass].colorAttachments[SceneColorAttachmentIndex].view;
}
...
// call). On an id mismatch, also checks expectedColorView (if given)
// against current_pass_color_view(): resolve_pass_into()'s substitution
// always carries the same colorView forward, so a still-matching view
// means the real render target survived and this resolves normally
// instead of refusing. Only when the id AND the colorView both no longer
// match (the real target is genuinely gone, not just re-wrapped) does it
// log a foreign-substitution warning and return false WITHOUT resolving/
// snapshotting the wrong pass. Treat a false return the same as
// resolve_pass() returning false.
bool resolve_pass_checked(const ResolveDesc& desc, ResolvedTargets& out, uint64_t expectedPassId,
  ...
  if (current_render_passes()[g_recorder.currentRenderPass].id != expectedPassId) {
    if (expectedColorView != nullptr && current_pass_color_view().Get() == expectedColorView.Get()) {
      ...
      Log.debug("resolve_pass_checked: pass id changed ({} -> {}) but colorView still matches -- "
                "render target survived an ordinary substitution, resolving normally.",
                expectedPassId, current_render_passes()[g_recorder.currentRenderPass].id);
      return resolve_pass(desc, out);
    }
    Log.warn("resolve_pass_checked: current pass (id={}) is not the pass (id={}) the caller opened -- "
             "something substituted it in the meantime. Refusing to resolve the wrong pass.",
             current_render_passes()[g_recorder.currentRenderPass].id, expectedPassId);
    if (g_recorder.inOffscreen) {
      // Recovery: the offscreen pass we opened was orphaned by a foreign
      // substitution (resolve_pass_into), which doesn't touch inOffscreen.
      ...
      // resync the offscreen-tracking state so finish()'s assert doesn't
      // fire on a pass that's already gone.
      g_recorder.inOffscreen = false;
      g_recorder.offscreenColor = {};
      g_recorder.offscreenDepth = {};
    }
...
// Desktop mirror support -- see gfx.hpp's doc comment. Lazily created once
// and cached; not tied to any particular source texture, so one instance
// covers every frame/eye this is ever called with.
